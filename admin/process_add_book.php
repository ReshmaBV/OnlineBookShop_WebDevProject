...
require('includes/config.php');
	...
	if(!empty($_POST))//IF SOMETHING IS SENT FROM addbook.php TO process_addbook.php THEN,
	{
		$msg=array();
		if(empty($_POST['name']) || empty($_POST['cat']) || empty($_POST['description']) || empty($_POST['publisher'])|| empty($_POST['edition']) || empty($_POST['isbn']) || empty($_POST['pages']) || empty($_POST['price']))
		{
		...
		if(!empty($msg))
		{
			echo '<b>There are some Errors while adding a book:-</b><br>';
			echo '<ul>';
			foreach($msg as $k)
			{
				echo '<li>'.$k.'</li>';
